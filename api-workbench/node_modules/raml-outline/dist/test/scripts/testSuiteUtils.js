...
var outlineInitializer = require("./outline-initializer");
...
function testOutline(apiPath, extensions, outlineJsonPath, regenerateJSON, callTests) {
...
function findNode(n, position, unitPath) {
    if (n == null) {
        return null;
    }
    ...
    if (lowLevel) {
        if ((lowLevel.start() <= position && lowLevel.end() >= position)
            ...
            var result = lowLevel.unit().absolutePath() == unitPath ? n : null;
...
function getDetailsData(apiPath, extensions) {
    var contents = fs.readFileSync(apiPath, "utf8");
    var folder = path.dirname(apiPath);
    var ramlFiles = iterateFolder(folder);
    var foundUnitPath = null;
    var foundPosition = null;
    for (var _i = 0, _a = ramlFiles[0].allRamlFiles(); _i < _a.length; _i++) {
        var ramlFile = _a[_i];
        var unitPath = ramlFile.absolutePath();
        var contents = fs.readFileSync(unitPath, "utf8");
...
function getDetailsJSON(api, selectedNode) {
    var highLevelNode = api.highLevel();
    var astProvider = {
        getASTRoot: function () {
    ...
    outlineInitializer.initialize2(astProvider);
    var json = index.getDetailsJSON();
    //json["nodeId"] = astProvider.getSelectedNode().id();
    return json;
}
function testDetails(apiPath, extensions, detailsJsonPath, regenerateJSON, callTests) {
    if (regenerateJSON === void 0) { regenerateJSON = false; }
    if (callTests === void 0) { callTests = true; }
    if (apiPath) {
        apiPath = data(apiPath);
