...
function addExampleControl(property, node, exampleElement, example, container) {
    if (example.isYAML()) {
        container.addItemToCategory(category(property, node), new itemsImpl.LowLevelTreeField(property, node, exampleElement.lowLevel(), example.name()));
    }
    else if (example.isJSONString()) {
        container.addItemToCategory(category(property, node), new itemsImpl.ExampleField(property, node, example.asString(), example.name()));
    }
    else if (example.isXMLString()) {
        container.addItemToCategory(category(property, node), new itemsImpl.XMLExampleField(property, node, example.asString(), example.name()));
    }
}
/**
 ...
 */
function buildItemByPosition(position) {
    return buildItem(findNode(position));
}
exports.buildItemByPosition = buildItemByPosition;
 ...
 */
function buildItem(parseResults) {
    if (!parseResults)
        return null;
    if (!parseResults.asElement())
        return null;
    var node = parseResults;
    ...
    props.forEach(function (x) {
        if (x.isValueProperty()) {
            ...
            if (nm && typeof nm.value() === "object") {
                result.addItemToCategory(category(x, node), new itemsImpl.StructuredField(x, node, nm.value()));
                return;
            }
...
//# sourceMappingURL=detailsImpl.js.map